...
CREATE TABLE trace_meta (
    ...
    expression INT NOT NULL,  -- Reference to expression table by index.
);

CREATE TABLE mapping_meta {
    src_expression INT NOT NULL,  -- Reference to expression table by index.
    tgt_expression INT NOT NULL,  -- Reference to expression table by index.

    src_mapping INT, -- Reference to expression table by index.
    tgt_mapping INT, -- Reference to expression table by index.
);


-- Created dynamically, one per expression.  Referenced by integer id that gets appended to the name.
CREATE TABLE expression_? (
    data ?,
);
