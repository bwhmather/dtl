...
class TableExpressionCompiler : public dtl::ast::TableExpressionVisitor {
  ...
  public:
    ...
    visit_select_expression(
        const dtl::ast::SelectExpression& expr) override final {
    ...
    visit_import_expression(
        const dtl::ast::ImportExpression& expr) override final {
    ...
    visit_table_reference_expression(
        const dtl::ast::TableReferenceExpression& expr) override final {
    ...
    std::shared_ptr<Scope>
    run(const dtl::ast::TableExpression& expr) {
        expr.accept(*this);
...
class StatementCompiler : public dtl::ast::StatementVisitor {
  ...
  public:
    ...
    visit_assignment_statement(
        const dtl::ast::AssignmentStatement& statement) override final {
        auto expr_table =
    ...
    visit_update_statement(
        const dtl::ast::UpdateStatement& statement) override final {
        (void)statement;
        assert(false);
    };
    ...
    visit_delete_statement(
        const dtl::ast::DeleteStatement& statement) override final {
        (void)statement;
        assert(false);
    };
    ...
    visit_insert_statement(
        const dtl::ast::InsertStatement& statement) override final {
        (void)statement;
        assert(false);
    };
    ...
    visit_export_statement(
        const dtl::ast::ExportStatement& statement) override final {
        auto expr_table =
            compile_table_expression(*statement.expression, m_context);
...
static void
compile_statement(const dtl::ast::Statement& statement, Context& context) {
